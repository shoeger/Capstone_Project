...
Partial Class Contact
    ...
    Protected Sub sendMail_Click(sender As Object, e As EventArgs) Handles sendMail.Click
        ...
        sendersName = name.Value.ToString() 'capture the senders name from the textbox
        sendersEmail = email.Value.ToString() 'capture the senders email from the textbox
        sendersSubject = subject.Value.ToString() 'capture the senders subject from the textbox
        sendersMessage = message.Value.ToString().Replace(Environment.NewLine, "<br />")   ' get the text from the message textbox keep text box formatting by replacing new lines (returns) with <br/>

        msg.To.Add("scott-thompson@uiowa.edu")  'destination email address
